...
<div>
         ...
         <?php 
         if(($_GET['viewreport']=='success')&&(!empty($_GET['id_report']))&&(!empty($_GET['id_user']))):
       ...
       if($inforeport['justificatif']==''){ 
        ...
        echo '<p>Absense de justificatif</p>';
         ...
         echo '<p id="'.$inforeport['id_report'].'"><a class="btn btn-success" href="'.$inforeport['justificatif'].'" target="_blank"><i class="icon-arrow-down icon-white"></i>Télécharger le justilicatif</a><a class="btn btn-info" href="#" id="Extrare_excel"><i class=" icon-arrow-down icon-white"></i>Report En EXCEL</a></p>';
       }
      ?>
...
<!--         <a class="btn btn-primary" id="valid_report_vivre">Livrer</a> -->
        <a class="btn btn-danger" href="travel_expensive.php?travel_encour=3" >Retour</a>
        </div>
    </div>
    </div>
          <?php 
